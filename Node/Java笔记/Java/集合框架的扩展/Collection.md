...
- Collection（（接口）存放简单的API操作，存着一组   不唯一且  无序  的对象）
	...
	- List（接口）
		...
		- ArrayList（底层数据结构是  数组  ）
			- 优点：遍历和随机访问速度快（查）
			- 缺点：添加和删除 速度慢（增删）
			- ArrayList实现类重写了Iterable方法，可以不用下标就可以遍历数组
			- 相关方法：
				- add
				- get
				- size
	...
	- Set（接口）
		...
		- 重复的数据指  值和地址都相等
		- 其唯一是比较引用地址和内容，当创建两个对象时，引用地址是不一样的，若内容一样，则会重复，重写equal和HashCode即可[[（十分重要）集合框架（java容器,集合类）]]
		- HashSet （默认值为16，本质上是HashMap）
			- 特点：
				...
				- hashset是set的hash的实现，值不能重复是hashmap中的key来实现，用hashcode方法来判断hashcode是否存在，如果存在则调用equals方法
				- 为了保证hashset的对象不会出现重复，该类要重写hashcode和equals方法
