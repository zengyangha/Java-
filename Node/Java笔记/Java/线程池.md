...
## 线程池的几种方式
- newFixedThreadPool()
	- 创建一个固定长度的线程池，每提交一个任务就创建一个线程，直到线程池到最大数量，此时线程规模不再变化，当线程异常结束时，线程池会补充一个新的线程
- newCachedThreadPool()
	- 可缓存的线程池，如果超过了线程规模，则自动回收空的线程，线程池规模没有限制
- newSingleThreadExecutor(),
	- 单线程的Executor，创建单个工作线程来执行任务，如果线程异常结束，则创建新的线程来替换，能保证按照任务在队列中，以顺序开串行来执行newSingleThreadExecutor（）
- newScheduledThreadPool()
	- 创建长度固定的线程池，以延迟或者定时的方式来执行任务，类似Timer
